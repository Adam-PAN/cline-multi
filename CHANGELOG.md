...
- Friendly Chinese error cards: ModelNotFound, ContentFiltered, ServerError, Timeout, Network errors now show clear descriptions and suggestions instead of raw error messages
- Real-time Token/Cost display bar in chat footer showing input tokens, output tokens, cache reads, and total cost
- Per-profile price configuration: each API profile now independently stores and restores its model pricing (inputPrice/outputPrice)
- Price input fields added for DeepSeek provider (previously only available for OpenAI Compatible)
- Error rewrite: DeepSeek/Mimo "DSML" tag errors and `search_files` missing regex parameter are now handled gracefully with retry logic

### Fixed

- Fix issue #1: model input/output prices were shared across all profiles — switching profiles now correctly restores per-profile pricing
- Fix "400 Invalid assistant message" errors for DeepSeek V4 by ensuring assistant messages always have content
- Fix reasoning-only models (DeepSeek V4, Mimo) returning empty responses by falling back to reasoning content
- Fix `tool_choice` from "required" to "auto" for DeepSeek/OpenAI Compatible/Qwen reasoning models
...
- Add `thinkingBudgetTokens` support for DeepSeek thinking models
